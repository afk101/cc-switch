...
use crate::commands::sync_support::{attach_post_operation_sync_result, run_post_import_sync};
...
pub async fn webdav_sync_download(state: State<'_, AppState>) -> Result<Value, String> {
    ...
    let sync_result = run_download_with_webdav_lock(
        ...
        |result| async move {
            let post_sync_result = run_post_import_sync(&app_state_for_sync).await;
    ...
    if let Some(msg) = post_sync_result.warning.as_ref() {
        log::warn!("[WebDAV] post-download sync warning: {msg}");
    }
    result = attach_post_operation_sync_result(result, post_sync_result);
