...
/// 判断 Codex Responses 输入是否包含带图片的工具输出。
pub fn contains_responses_tool_output_images(body: &Value) -> bool {
    match body.get("input") {
        Some(Value::Array(items)) => items.iter().any(responses_tool_output_has_images),
        Some(item @ Value::Object(_)) => responses_tool_output_has_images(item),
        _ => false,
    }
}

/// 判断单个 Responses 输入项的工具输出是否包含图片。
fn responses_tool_output_has_images(item: &Value) -> bool {
    item.get("output")
