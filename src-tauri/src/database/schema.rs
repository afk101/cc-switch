...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    13 => {
                        log::info!("迁移数据库从 v13 到 v14（汇合 Profile Schema 并添加 Grok Build 代理配置）");
                        Self::migrate_v13_to_v14(conn)?;
                        Self::set_user_version(conn, 14)?;
                    }
                    14 => {
                        log::info!(
                            "迁移数据库从 v14 到 v15（Profile 路由补偿与 Grok Build Skills/MCP）"
                        );
                    ...
                    15 => {
                        log::info!(
                            "迁移数据库从 v15 到 v16（校准完整 Schema 并重建 Codex 会话用量）"
                        );
                    ...
                    16 => {
                        log::info!(
                            "迁移数据库从 v16 到 v17（补齐 Grok Build 开关与会话用量去重账本）"
                        );
    ...
    /// 幂等补齐 Grok Build 独立代理配置行。
    fn ensure_grokbuild_proxy_schema(conn: &Connection) -> Result<(), AppError> {
        if !Self::table_exists(conn, "proxy_config")? {
            return Ok(());
        }
    ...
    /// 幂等补齐 Grok Build 的统一 Skills 与 MCP 开关。
    fn ensure_grokbuild_skill_mcp_schema(conn: &Connection) -> Result<(), AppError> {
    ...
    /// 幂等补齐会话用量持久去重账本。
    fn ensure_session_usage_dedup_schema(conn: &Connection) -> Result<(), AppError> {
        conn.execute_batch(
    ...
    /// v13 -> v14：汇合 Profile 分支并添加 Grok Build 代理配置。
    fn migrate_v13_to_v14(conn: &Connection) -> Result<(), AppError> {
        Self::reconcile_profile_schema_branches(conn)?;
        Self::ensure_grokbuild_proxy_schema(conn)
    }

    /// v14 -> v15：补齐 Profile 补偿字段及 Grok Build Skills/MCP 开关。
    ...
    /// v15 -> v16：校准完整 Schema，并重建 Codex 会话用量。
    ...
    /// v16 -> v17：汇合历史同版本分支的 Grok Build 开关与会话用量去重账本。
