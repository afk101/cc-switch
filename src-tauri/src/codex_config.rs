...
/// Profile 路由接管使用的临时 provider id 前缀。
///
/// 独立 provider 能把本地 listener token 与用户原有供应商认证隔离，避免
/// Codex 0.149 将顶层 token 忽略，或被原表的 env/header 认证覆盖。
pub(crate) const CC_SWITCH_CODEX_PROFILE_PROXY_PROVIDER_ID: &str = "cc-switch-profile";
/// Profile 临时 provider 的固定显示名，也用于恢复阶段识别应用自建空壳表。
pub(crate) const CC_SWITCH_CODEX_PROFILE_PROXY_PROVIDER_NAME: &str = "CC Switch Profile";
...
pub fn prepare_codex_config_with_model_catalog(
    ...
    if let Some(catalog) = codex_model_catalog_from_settings(settings, config_text, profile)? {
        ...
        Ok(PreparedCodexConfigWithModelCatalog {
            config_text: set_codex_native_web_search_field(
                &config_text,
...
/// 从指定前缀选择首个未占用的 provider id，避免覆盖用户手写表。
fn first_free_provider_id(
    base_id: &str,
    model_providers: Option<&dyn toml_edit::TableLike>,
) -> String {
...
fn first_free_cc_switch_provider_id(model_providers: Option<&dyn toml_edit::TableLike>) -> String {
    first_free_provider_id(CODEX_MIGRATED_PROVIDER_ID, model_providers)
}
...
/// 判断现有自定义 provider 能否安全承载 Profile listener token。
///
/// `auth` 会与 bearer token 形成反序列化冲突，`env_key` 会在运行时抢占 token，
/// `aws` 只属于 Bedrock 内置表。其他字段可原样保留并在关闭路由时恢复。
fn codex_profile_provider_can_accept_listener_token(provider: &dyn toml_edit::TableLike) -> bool {
    provider.get("auth").is_none()
        && provider.get("env_key").is_none()
        && provider.get("aws").is_none()
}

/// 为 Codex Profile 构造本地代理 provider，并隔离不兼容的用户认证字段。
///
/// Codex 0.149 只读取 provider 表内的 `experimental_bearer_token`。因此 Profile
/// 路由不能把 listener token 写到顶层。普通自定义表继续沿用原路径，保证历史
/// 备份可恢复；带 `auth` / `env_key` 的表以及内置 provider 则使用独立临时表。
/// 恢复阶段会在所有权校验通过后删除临时表并还原原选择器。
pub(crate) fn apply_codex_profile_proxy_route(
    config_text: &str,
    proxy_base_url: &str,
    listener_token: &str,
) -> Result<String, AppError> {
    preflight_codex_provider_table_conflicts(config_text)?;

    // 0.149 会校验所有自定义表；即使不是当前活动表，缺少 name 也会让整份配置失效。
    let named = backfill_codex_custom_provider_names(config_text)?;
    let normalized = named.as_deref().unwrap_or(config_text);
    let mut doc = normalized
        .parse::<DocumentMut>()
        .map_err(|error| AppError::Message(format!("Invalid Codex config.toml: {error}")))?;

    if let Some((reserved_id, _)) = CODEX_STALE_RESERVED_TABLE_IDS
        .iter()
        .find_map(|reserved_id| {
            doc.get("model_providers")
                .and_then(|item| item.as_table_like())
                .and_then(|providers| providers.get(reserved_id))
                .and_then(|item| item.as_table_like())
                .map(|table| (*reserved_id, table))
        })
    {
        return Err(AppError::localized(
            "provider.codex.config.reserved_provider_table",
            format!(
                "Codex 0.149 禁止覆盖内置 provider `{reserved_id}`；请先保存或切换一次该供应商，让 CC Switch 将旧表迁移为自定义 provider 后再启用 Profile 路由"
            ),
            format!(
                "Codex 0.149 forbids overriding the built-in provider `{reserved_id}`. Save or switch the provider once so CC Switch can migrate the stale table before enabling the Profile route"
            ),
        ));
    }
    ...
    {
        return Err(AppError::localized(
            "provider.codex.config.invalid_model_providers",
            "Codex Profile 路由要求 model_providers 为 TOML 表，当前配置无法安全追加本地路由",
            "Codex Profile routing requires model_providers to be a TOML table; the local route cannot be added safely",
        ));
    }

    // 普通自定义表维持旧路径；已接管 Home 也会复用原临时 id，避免每次对账生成后缀。
    let reusable_route_provider_id = active_codex_model_provider_id(&doc).filter(|provider_id| {
        is_custom_codex_model_provider_id(provider_id)
            && doc
                .get("model_providers")
                .and_then(|item| item.as_table_like())
                .and_then(|providers| providers.get(provider_id))
                .and_then(|item| item.as_table_like())
                .is_some_and(codex_profile_provider_can_accept_listener_token)
    });
    let reuses_existing_provider = reusable_route_provider_id.is_some();
    let route_provider_id = reusable_route_provider_id.unwrap_or_else(|| {
        first_free_provider_id(
            CC_SWITCH_CODEX_PROFILE_PROXY_PROVIDER_ID,
            doc.get("model_providers")
                .and_then(|item| item.as_table_like()),
        )
    });
    ...
    let providers = doc
        ...
        .and_then(|item| item.as_table_like_mut())
        .ok_or_else(|| {
            AppError::Config("Codex Profile 路由无法创建 model_providers 表".to_string())
        })?;
    // 旧全局接管占位符由 CC Switch 自己生成；切到 Profile 私有 token 时必须清掉残留。
    remove_codex_proxy_placeholders_from_providers(providers);

    if reuses_existing_provider {
        let route = providers
            .get_mut(&route_provider_id)
            .and_then(|item| item.as_table_like_mut())
            .ok_or_else(|| {
                AppError::Config("Codex Profile 路由无法读取活动 provider 表".to_string())
            })?;
        route.insert("base_url", toml_edit::value(proxy_base_url));
        route.insert("wire_api", toml_edit::value("responses"));
        route.insert(
            "experimental_bearer_token",
            toml_edit::value(listener_token),
        );
    } else if container_is_inline {
        let mut route = toml_edit::InlineTable::new();
        route.insert("name", CC_SWITCH_CODEX_PROFILE_PROXY_PROVIDER_NAME.into());
        route.insert("base_url", proxy_base_url.into());
        route.insert("wire_api", "responses".into());
        route.insert("experimental_bearer_token", listener_token.into());
        providers.insert(
            &route_provider_id,
            toml_edit::Item::Value(toml_edit::Value::InlineTable(route)),
        );
    } else {
        let mut route = toml_edit::Table::new();
        route.insert(
            "name",
            toml_edit::value(CC_SWITCH_CODEX_PROFILE_PROXY_PROVIDER_NAME),
        );
        route.insert("base_url", toml_edit::value(proxy_base_url));
        route.insert("wire_api", toml_edit::value("responses"));
        route.insert(
        ...
        providers.insert(&route_provider_id, toml_edit::Item::Table(route));
    }
    doc["model_provider"] = toml_edit::value(route_provider_id);
    Ok(doc.to_string())
}
...
fn remove_codex_proxy_placeholders_from_providers(providers: &mut dyn toml_edit::TableLike) {
    ...
    fn catalog_infers_image_input_independently_of_tool_profile() {
        ...
        let specs = vec![
            ...
            CodexCatalogModelSpec {
                ...
                display_name: Some("360 GLM 5.2".to_string()),
                context_window: Some(128_000),
                supports_parallel_tool_calls: None,
                input_modalities: None,
                base_instructions: None,
            ...
            CodexCatalogModelSpec {
                ...
                display_name: Some("360 DeepSeek V4 Flash".to_string()),
                context_window: Some(128_000),
                supports_parallel_tool_calls: None,
                input_modalities: None,
                base_instructions: None,
            ...
            CodexCatalogModelSpec {
                ...
                display_name: Some("360 GLM 5.2V".to_string()),
                context_window: Some(128_000),
                supports_parallel_tool_calls: None,
                input_modalities: None,
                base_instructions: None,
