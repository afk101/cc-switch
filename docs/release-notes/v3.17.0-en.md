...
- **Independent model-catalog sync for every Codex Profile Home**: every `CODEX_HOME` gets its own generated `cc-switch-model-catalog.json`, so no symlink to the default `~/.codex` copy is needed. Provider configuration remains globally shared; editing a provider transactionally updates every Home that uses it as its primary provider, while enabling or switching one Profile affects only the selected Home. Switching to a provider without a catalog removes only CC Switch's `model_catalog_json` pointer and keeps the existing JSON file for future reuse. Restart the corresponding Codex instance to reload catalog changes.
