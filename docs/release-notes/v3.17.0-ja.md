...
- **各 Codex Profile Home のモデルカタログを独立同期**：各 `CODEX_HOME` に専用の `cc-switch-model-catalog.json` を生成するため、既定の `~/.codex` にあるファイルへのシンボリックリンクは不要です。プロバイダー設定は引き続きグローバル共有され、プロバイダーを編集すると、それを主プロバイダーとして参照するすべての Home がトランザクション的に更新されます。一方、1 つの Profile の有効化・切り替えは選択した Home だけに作用します。カタログを持たないプロバイダーへ切り替えた場合は、CC Switch 管理の `model_catalog_json` ポインターだけを削除し、既存の JSON ファイルは将来の再利用のため残します。変更したカタログを再読み込みするには、対応する Codex を再起動してください。
