...
- **每个 Codex Profile Home 独立同步模型目录**：每个 `CODEX_HOME` 都会生成自己的 `cc-switch-model-catalog.json`，不再需要把默认 `~/.codex` 下的文件软链接过去。供应商配置仍全局共享；编辑供应商时会事务式更新所有把它作为主供应商的 Home，单独启用或切换 Profile 则只影响所选 Home。切到没有模型目录的供应商时，只移除 CC Switch 管理的 `model_catalog_json` 指针，已有 JSON 文件保留以便以后复用。目录变更后需重启对应 Codex 才会重新加载。
