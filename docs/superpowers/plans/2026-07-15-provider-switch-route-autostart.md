...
- [x] **Step 6: 运行 home config 测试确认 GREEN**
...
- [x] **Step 1: 编写 provider requirement 纯函数测试**
...
- [x] **Step 4: 现场安全状态与运行时边界验收**

自动化 manager/runtime 用例验证：官方订阅切换保持路由关闭且不创建 token/runtime；需要路由的 provider 只提示；显式路由开关仍独占 enable 调用。为避免再次污染用户 Home，本轮不重启旧开发进程；交互 UI 由用户使用新构建复验。现场 `~/.codex-api` 已确认恢复为订阅直连且 15722 无监听。
